...
6. Produce the exact `NORDVELL_EMAIL_PACKAGE` defined in the email standard on the first response. Include a complete plain-text version and complete HTML document, not a wireframe, partial snippet, or prose description.
   The `html` field must contain the authored document in full. Never substitute a plan, file reference, example reference, module outline, or instructions for the HTML source.
...
1. Select the write mode. For a normal write, ask one consolidated activation question, regenerate in `ajo-ready`, and fail closed if a placeholder remains. If the user explicitly requests `poc-draft-write`, preserve allowed placeholders, set `pocDraft: true`, `productionReady: false`, `readyToCreateDraft: true`, and label the template `POC DRAFT - NOT FOR SEND`.
...
In `poc-draft-write`, creation or update of the draft Content Template is allowed with the approved placeholder manifest. Do not apply it to a message, publish related fragments, bind a policy, send a proof, activate, or send. The receipt must list every placeholder and POC scenario assumption.
