...
Do not require the user to understand the complete contract before helping. Extract available facts from their request and relevant read tools. Default to `creative-draft` mode and do not ask questions before producing the first complete visual draft. Use `bright-editorial-v1` and approved non-sendable placeholders for missing activation details.

## Workflow

1. Separate supplied/approved, tool-verified, drafting-only, and unresolved facts according to the campaign facts contract.
2. Verify that customer-facing statements are supported. The current MCP has no inventory tool; never imply that an AJO read verified stock. Missing stock or eligibility evidence cannot be replaced by a placeholder; choose truthful fallback copy instead.
...
7. Before a requested AJO write, produce one consolidated `activationQuestion` covering all unresolved facts. After the user answers, regenerate in `ajo-ready` mode and require zero placeholders before mutation preflight.
